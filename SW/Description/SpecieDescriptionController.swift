...
class SpecieDescriptionController: UITableViewController {
    
    private var elements = [DescriptionElement]()
    
    func setObjectId(_ id: Int16) {
        DataRepository.shared.fetchSpecie(with: id) { [weak self] specie in
            self?.elements = DescriptionBuilder().build(from: specie)
